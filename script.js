...
function showQuestion() {
  if (currentQuestionIndex < questions.length) {
    ...
    for (var i = 0; i < currentQuestion.options.length; i++) {
      optionsHtml += '<input type="radio" name="answer" value="' + currentQuestion.options[i] + '" onclick="selectAnswer(this)">  ' + currentQuestion.options[i] + '<br>';
    }
...
function checkAnswer() {
  if (selectedAnswer !== null) {
    ...
    } while (currentQuestionIndex < questions.length && askedQuestions.includes(questions[currentQuestionIndex]));
